...
function openChargeModal() {
  ...
  document.getElementById('charge-modal-dyn')?.remove();
  const overlay = document.createElement('div');
  overlay.id = 'charge-modal-dyn';
  ...
  const optStyle = (sel) => sel
    ? 'border:2px solid #e8850a;border-radius:16px;padding:14px 16px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#fff8ed'
    : 'border:1.5px solid rgba(0,0,0,.12);border-radius:16px;padding:14px 16px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;background:#fff';
  overlay.innerHTML = `<div style="background:#fff;border-radius:24px 24px 0 0;padding:24px 20px 44px;width:100%;max-width:390px">
    <div style="width:40px;height:4px;background:rgba(0,0,0,.12);border-radius:2px;margin:0 auto 20px"></div>
    <div style="font-size:17px;font-weight:700;text-align:center;margin-bottom:4px">残高をチャージ</div>
    <div style="font-size:12px;color:#b0aea6;text-align:center;margin-bottom:20px">チャージした残高で票や上場枠を購入できます</div>
    <div id="copt-500" style="${optStyle(false)}">
      <div><div style="font-family:monospace;font-size:18px;font-weight:700">¥500</div><div style="font-size:11px;color:#b0aea6">ボーナスなし</div></div>
      <span style="font-family:monospace;font-size:13px;color:#b0aea6">500円分</span>
    </div>
    <div id="copt-1000" style="${optStyle(false)}">
      <div><div style="font-family:monospace;font-size:18px;font-weight:700">¥1,000</div><div style="font-size:11px;color:#1a7a4a;font-weight:700">+50円ボーナス</div></div>
      <span style="font-size:12px;color:#bf6d00;font-weight:700">1,050円分</span>
    </div>
    <div id="copt-3000" style="${optStyle(false)}">
      <div><div style="font-family:monospace;font-size:18px;font-weight:700">¥3,000</div><div style="font-size:11px;color:#1a7a4a;font-weight:700">+300円ボーナス 🎉</div></div>
      <span style="font-size:12px;color:#bf6d00;font-weight:700">3,300円分</span>
    </div>
    <button id="charge-start-btn" style="margin-top:12px;width:100%;padding:15px;background:#ccc;border:none;border-radius:16px;font-size:15px;font-weight:700;color:#fff;cursor:pointer;font-family:sans-serif">金額を選択してください ↑</button>
    <button id="charge-cancel-btn" style="width:100%;padding:12px;background:transparent;border:none;font-size:14px;color:#b0aea6;cursor:pointer;font-family:sans-serif;margin-top:4px">キャンセル</button>
  </div>`;
  document.body.appendChild(overlay);
  overlay.addEventListener('click', e => { if (e.target === overlay) closeChargeModal(); });
  document.getElementById('copt-500').addEventListener('click', () => selectCharge(500));
  document.getElementById('copt-1000').addEventListener('click', () => selectCharge(1000));
  document.getElementById('copt-3000').addEventListener('click', () => selectCharge(3000));
  document.getElementById('charge-start-btn').addEventListener('click', startCharge);
  document.getElementById('charge-cancel-btn').addEventListener('click', closeChargeModal);
}
...
function selectCharge(amount) {
  ...
  ['500','1000','3000'].forEach(a => {
    const el = document.getElementById('copt-' + a);
    if (!el) return;
    if (a === String(amount)) {
      el.style.border = '2px solid #e8850a';
      el.style.background = '#fff8ed';
    } else {
      el.style.border = '1.5px solid rgba(0,0,0,.12)';
  ...
  if (btn) { btn.style.background = '#e8850a'; btn.textContent = labels[amount]; }
}
...
async function startCharge() {
  ...
  try {
    ...
    if (json.url) { window.location.href = json.url; }
      ...
      showToast('エラー: ' + msg);
      if (btn) { btn.style.background = '#e8850a'; btn.textContent = '再試行する →'; }
    }
  } catch(e) {
